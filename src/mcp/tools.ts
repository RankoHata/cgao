...
import {
  getIssue, listIssues, addComment,
  getPR, listPRs, getPRStatus, getPRChecks, listReviews,
...
interface ToolDef {
  ...
  handler: (a: Record<string, unknown>) => Promise<{ content: Array<{ type: 'text'; text: string }>; isError?: boolean }>;
}
function ok(t: string) { return { content: [{ type: 'text' as const, text: t }] }; }
function err(t: string) { return { content: [{ type: 'text' as const, text: t }], isError: true }; }
...
Use BEFORE planning any fix — this is the mandatory evaluation gate.`,
    ...
    handler: async (args) => {
      try {
        ...
        const rOwner = (args.owner as string) || owner;
        const rRepo = (args.repo as string) || repoName;
        ...
        else if (issue.assignee) recommendation = 'skip_assigned';
...
Use AFTER triage to understand what needs to change.`,
    ...
    handler: async (args) => {
      try {
        ...
        const findings: Array<{ term: string; matches: number; files: string[] }> = [];
        for (const term of allTerms.slice(0, 10)) {
          ...
          } catch { findings.push({ term, matches: 0, files: [] }); }
        }
    ...
    handler: async (args) => {
      try {
        ...
        const plan = {
          ...
          steps: [] as Array<{ order: number; description: string; files: string[]; estimated_complexity: string; test_strategy: string }>,
        ...
        // Docs
        if (triage && (triage as Record<string, unknown>).classification === 'feature') {
...
blocker report and recommended action. Use for monitoring PRs.`,
    ...
    handler: async (args) => {
      try {
        ...
        const rOwner = (args.owner as string) || o;
        const rRepo = (args.repo as string) || r;
