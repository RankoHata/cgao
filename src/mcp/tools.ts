...
// ---- State helpers ----
const STATE_DIR =
  process.env.CGAO_STATE_DIR || join(process.cwd(), '.cgao');

function ensureStateDir() {
  if (!existsSync(STATE_DIR)) mkdirSync(STATE_DIR, { recursive: true });
}
function readState(key: string): Record<string, unknown> | null {
...
function writeState(key: string, data: Record<string, unknown>) {
  ensureStateDir();
  writeFileSync(
    join(STATE_DIR, `${key}.json`),
    JSON.stringify({ ...data, _updated: new Date().toISOString() }, null, 2),
  );
}
...
This tool does NOT call GitHub's API — it only analyzes data you provide.`,
    schema: {
      ...
      properties: {
        issue_number: {
          type: 'number',
          description:
            'Issue number (used for state file naming).',
        },
        issue: {
          type: 'object',
          description:
            'Full issue object from mcp__github__issue_read or mcp__github__search_issues. ' +
    ...
    handler: async (args) => {
      try {
        ...
        const labels = arr(issue.labels).map((l) =>
        ...
        const isBug =
          labels.includes('bug') ||
          /\b(bug|broken|error|crash|fail|incorrect|wrong|regression)\b/.test(
            title + ' ' + body,
          );
        const isFeature =
          labels.includes('enhancement') ||
          labels.includes('feature') ||
          /\b(feature request|enhancement|add support for|please add)\b/.test(
            title + ' ' + body,
          );
        const isQuestion =
          /\b(how do i|how to|what is|question|help)\b/.test(
            title + ' ' + body,
          ) && !isBug;
        ...
        let severity = 'medium';
        if (
          /\b(security|vulnerability|xss|sql injection|auth|permission|data leak|exposure)\b/.test(
            title + ' ' + body,
          )
        )
          severity = 'critical';
        else if (
          /\b(crash|segfault|panic|deadlock|race condition|memory leak|corruption)\b/.test(
            title + ' ' + body,
          )
        )
          severity = 'high';
        ...
        let scope = 'unknown';
        const fileRefs =
          body.match(/`?([\w/.-]+\.(ts|js|py|go|rs|java|rb))`?/g) || [];
        if (
          fileRefs.length > 5 ||
          /\b(multi|several|many files|large refactor|architecture|restructure)\b/.test(
            body,
          )
        )
          scope = 'large';
        ...
        // ---- Decision ----
        const actionable =
          (isBug || isFeature) && !isQuestion && state === 'open';
        ...
        const result = {
          ...
          summary: `${
            ...
            recommendation === 'fix'
              ? '✅ SHOULD FIX'
              : '⏭️ SKIP'
          } — ${severity} severity ${isBug ? 'bug' : isFeature ? 'feature' : 'issue'} (${scope} scope)`,
        };
  ...
  3. This tool searches the LOCAL filesystem and git history`,
    schema: {
      ...
      properties: {
        ...
        issue_body: {
          type: 'string',
          description:
            'Full issue body text. Used to extract file names, function names, error messages.',
        },
        search_terms: {
          type: 'array',
          items: { type: 'string' },
          description:
            'Additional search terms to grep for in the codebase.',
        },
      },
    ...
    handler: async (args) => {
      try {
        ...
        const extraTerms = (args.search_terms as string[]) || [];
...
Use AFTER triage and codebase analysis, BEFORE implementation.`,
    ...
    handler: async (args) => {
      try {
        const num = args.issue_number as number;
        const approach = args.fix_approach as string;
        const affected = (args.affected_files as string[]) || [];
        ...
        plan.steps.push({
          order: ++order,
          description:
            'Add or update tests to cover the fix and prevent regression',
          files: [],
        ...
        if (
          triage &&
          (triage as Record<string, unknown>).classification === 'feature'
        ) {
          plan.steps.push({
            order: ++order,
            description:
              'Update documentation if this is a user-facing change',
...
This tool uses only LOCAL git operations — no GitHub API calls.`,
    schema: {
      ...
      properties: {
        pr_number: {
          type: 'number',
          description:
            'PR number (optional — checks current branch if omitted).',
        },
        branch: {
          type: 'string',
    ...
    handler: async (args) => {
      try {
        ...
        const prNum = args.pr_number as number | undefined;

        let headBranch = args.branch as string | undefined;
        if (!headBranch) {
        ...
        const changedFiles = shell(
          ...
          .split('\n')
          .filter(Boolean);
        const suspicious = changedFiles.filter((f) =>
          unrelatedPatterns.some((p) => f.includes(p)),
        );
        ...
        const hasConventional = commits.some((c) =>
          /^(fix|feat|chore|docs|refactor|test|style|perf)(\(.+\))?:/.test(c),
        );
        if (hasConventional && hasIssueRef)
          ...
          checks.push({
            ...
            status: 'WARN',
            detail:
              'Consider using conventional commits (fix:/feat:) and referencing the issue',
          });
        ...
        if (testFiles.length > 0)
          ...
          checks.push({
            ...
            status: 'WARN',
            detail:
              'No test files detected in diff — consider adding tests',
          });
...
Then pass all results to this tool for analysis.`,
    schema: {
      ...
      properties: {
        ...
        pr: {
          type: 'object',
          description:
            'Full PR object from mcp__github__pull_request_read (method: "get"). ' +
            'Must include: title, state, head ({ref, sha}), base ({ref}), html_url, user ({login}), draft, mergeable, merged_at.',
        },
        reviews: {
          type: 'array',
          description:
            'Reviews array from mcp__github__pull_request_read (method: "get_reviews"). ' +
            'Each review should have: state, user ({login}), submitted_at.',
        },
        ci_status: {
          type: 'object',
          description:
            'CI status from mcp__github__pull_request_read (method: "get_status"). Optional. ' +
            'Should include: state, statuses (array of {context, state, description}).',
        },
        check_runs: {
          type: 'array',
          description:
            'Check runs from mcp__github__pull_request_read (method: "get_check_runs"). Optional. ' +
    ...
    handler: async (args) => {
      try {
        ...
        const pr = args.pr as Record<string, unknown> | undefined;

        if (!pr) {
          return err(
            'Missing required parameter "pr". ' +
        ...
        const prMergeable = pr.mergeable as boolean | null | undefined;
        const prMergedAt = pr.merged_at as string | null | undefined;
      ...
      try {
        ...
        if (action === 'set') {
          ...
          const updated = {
            ...
            phases: {
              ...((existing.phases as Record<string, string>) || {}),
              [args.phase as string]: new Date().toISOString(),
            },
          };
